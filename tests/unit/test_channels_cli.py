...
def test_channels_list(mock_config_manager):
    """Test 'tshell channels list' command."""
    runner = CliRunner()
...
def test_channels_add(mock_config_manager):
    """Test 'tshell channels add' command."""
    runner = CliRunner()
    result = runner.invoke(cli, ["channels", "add", "@new_channel"])
    
    assert result.exit_code == 0
    assert "Added @new_channel" in result.output
    
    # Verify save was called with the updated list
    args, _ = mock_config_manager.save.call_args
    assert "@new_channel" in args[0]["default_channels"]
    assert "@existing" in args[0]["default_channels"]
